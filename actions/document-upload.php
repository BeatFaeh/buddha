...
$redirect = static function () use ($flash) {
    header('Location: index.php?action=admin&notice=upload#dokument-upload');
    exit;
};
...
$areas = [
    'literatur' => __DIR__ . '/../literatur',
    'lernmodule' => __DIR__ . '/../lernmodule',
    'meditation' => __DIR__ . '/../meditation',
];
$labels = [
    'literatur' => 'Literatur',
...
if ($uploadError !== UPLOAD_ERR_OK) {
    ...
    error_log('Buddha Uploadfehler: PHP error=' . $uploadError . ', file=' . (string)($file['name'] ?? ''));
    $redirect();
}
...
$allowed = ['pdf','doc','docx','xls','xlsx','ppt','pptx','txt','md','jpg','jpeg','png','webp','mp3','wav','m4a','mp4','mov','webm','zip'];
...
if (!is_writable($targetDir)) {
    ...
    error_log('Buddha Uploadfehler: Ziel nicht beschreibbar: ' . $targetDir);
    $redirect();
}
...
if ($tmpName === '' || !is_uploaded_file($tmpName)) {
    ...
    error_log('Buddha Uploadfehler: tmp-Datei ungueltig: ' . $tmpName);
    $redirect();
}
...
if (!$stored || !is_file($target)) {
    ...
    error_log('Buddha Uploadfehler: Speichern fehlgeschlagen. target=' . $target . '; detail=' . $detail);
    $flash->set(
        'error',
